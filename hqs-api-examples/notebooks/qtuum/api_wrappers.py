...
class QuantinuumAPI:
    ...
    def __init__(self,
        ...
        self.user_name = self._get_val(config, key='user_name', kwarg_option=user_name, default=self._get_email())
    ...
    def _request_tokens(self, body, email=None):
        ...
        try:
            ...
            if response.status_code != HTTPStatus.OK:
                ...
                self._save_tokens(response.json()['id-token'], response.json()['refresh-token'], email)
    ...
    def _authenticate(self, action=None, email=None):
        ...
        status_code, message = self._request_tokens(body, user_name)
    ...
    def _get_email(self):
        """ Method to retrieve email  from system's keyring service """
        email = None
        email = keyring.get_password(self.keyring_service, 'email')
        return email

    
    def _save_tokens(self, id_token:str, refresh_token:str, email=None):
        ...
        # also save email for federated token refresh
        if email:
            keyring.set_password(self.keyring_service, 'email', email)
    ...
    def login(self) -> str:
        ...
        if expiration_date < (datetime.datetime.now(datetime.timezone.utc).timestamp() + self.time_safety_factor):
            ...
            if refresh_token is not None:
                self._authenticate('refresh', email)
            else:
                self._authenticate()
